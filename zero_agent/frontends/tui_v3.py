...
# Make `frontends/` parent (project root) importable so ZeroAgent imports work
# whether this file is run as `python -m frontends.tui_v3` or directly via
...
class SB:
    ...
    def _cmd(self, raw: str) -> None:
        ...
        #     self.commit([_t('err.multi_session', name=name)])
            ...
            # The agent expands `/resume` into a session-recovery prompt before
            # the LLM sees it.
            self._submit('/resume', [])
