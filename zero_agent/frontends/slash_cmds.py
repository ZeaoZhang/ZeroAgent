...
def build_update_prompt(args_text: str = "") -> str:
    ...
    if _current_lang() == "en":
        return (
            ...
            "   - If local commits exist: merge the remote tracking branch into the current branch and keep the local commits.\n"
            "   - Otherwise: fast-forward or pull from the configured remote while preserving uncommitted working-tree changes.\n"
    ...
    return (
        "请你更新当前 ZeroAgent 仓库，使用当前配置的 git remote。\n"
        "1. 先 fetch；识别当前分支，以及是否存在领先远端跟踪分支的本地 commit。\n"
        "2. 给出简洁预览：本地尚未包含的远端最近提交（短 hash + 标题 + 日期）及变更文件摘要。\n"
        "3. 执行更新（工作区未提交的改动始终要保留）：\n"
        "   - 若存在本地 commit：将远端跟踪分支合并进当前分支，并保留本地 commit。\n"
        "   - 否则：从当前配置的远端 fast-forward 或 pull，同时保留工作区未提交改动。\n"
        "4. 最后给出简短小结：分支 HEAD、变更文件、冲突/解决方式。\n"
        "\n"
        "#### 向用户说 'Congratulations! 🎉'\n"
        "更新成功后，请对用户说：\"Congratulations! 🎉 你已成功更新 ZeroAgent！\""
        f"{_tail(args_text)}"
    )


def build_init_prompt(args_text: str = "") -> str:
    """Prompt-only /init orchestration for ZA capability bootstrap.

    This is intentionally not the old GA `mykey.py` wizard.  ZA uses
    `config.yaml` plus memory/SOP docs; the agent should detect what already
    exists, initialize missing local docs/state, and only ask the user for
    browser-extension or credential choices that cannot be inferred safely.
    """
    return (
        "请执行 ZeroAgent 一键初始化。注意：这不是 GenericAgent 的 mykey.py 向导；"
        "ZA 使用 config.yaml 和 memory/SOP 文档。\n\n"
        "必须先读取并遵循这些初始化/能力文档：\n"
        "- memory/memory_management_sop.md\n"
        "- memory/web_setup_sop.md\n"
        "- memory/tmwebdriver_sop.md\n"
        "- memory/vision_sop.md\n"
        "- memory/vue3_component_sop.md（浏览器前端交互场景需要时）\n\n"
        "初始化目标：\n"
        "1. 识别当前 ZA 配置：定位 config.yaml，检查 default_backend、llm_backends、"
        "workspace_dir、memory_dir、sessions_dir、log_dir 等关键字段是否存在且路径可用；"
        "不要在聊天中泄露 API key，只报告是否缺失/占位。\n"
        "2. 初始化本地运行目录和记忆体系：workspace、sessions、memory L1/L2/L4、"
        "默认 SOP 文档；已有内容不能覆盖。\n"
        "3. 检查浏览器/Web 能力：确认 TMWebDriver/CDP bridge 相关文件是否存在，"
        "检查 web_setup_sop 中要求的扩展路径和浏览器可用性；能自动打开普通测试页/检查端口/检查文件的先自动做。\n"
        "4. 对缺失能力给出可执行修复：例如需要用户安装/启用 Chrome 扩展、选择浏览器、"
        "补充 config.yaml 的 provider/api_key/model 时，使用 ask_user 一次性询问关键选择；"
        "除这些必须用户参与的步骤外，不要停下来问。\n"
        "5. 初始化 ZA 的身份与长期画像：先检查 memory 中是否已有 ZA 自身定义、交互风格、"
        "用户画像、人像/头像偏好等稳定资料；如果没有明确资料，使用 ask_user 让用户直接输入一段自由文本来制定，"
        "可参考 OpenClaw 的 soul.md 形态，但不要假设本地存在该文件。用户也可以给来源路径；"
        "若用户给的是文本，按 memory/memory_management_sop.md 将稳定事实写入合适的 L2/L3 记忆"
        "（例如 ZA 自身定义、性格、交互边界、用户画像、人像/视觉描述）。不要凭空编造人格档案。\n"
        "6. 最后输出初始化报告：已完成、仍需用户支持、建议下一步；如果浏览器能力未验证通过，"
        "明确提示按 memory/web_setup_sop.md 执行。\n"
...
def list_launchable_services() -> list[dict]:
    ...
    Sources (hub.pyw EXCLUDES = goal_mode.py / chatapp_common.py / tuiapp.py):
      • reflect/*.py   (not '_'-prefixed, not excluded)
          → cmd = [python, -m, zero_agent.runners.cli, --reflect, reflect/<f>]
...
def _match_service(cmdline: list[str], svc: dict) -> bool:
    ...
    Reflect detection used to require BOTH the old monolithic entrypoint AND
    the reflect path in cmdline.  That rejected tasks launched directly
    (`python reflect/scheduler.py`) by launchers, dev scripts, or by an earlier
    TUI run that used a different launcher — they showed unticked in /scheduler
    even when alive.  Path-only match handles both styles; the
...
PALETTE_ENTRIES: list[tuple[str, str, str]] = [
    ("/init",      "[scope]",   "一键初始化配置、记忆、浏览器能力与 ZA 身份画像"),
    ("/update",    "[note]",    "git pull 更新 ZA 仓库并报告影响面"),
