    ...
    if os.path.isdir(reflect_dir):
        for f in sorted(os.listdir(reflect_dir)):
            if f.endswith('.py') and not f.startswith('_') and f not in EXCLUDES:
                services.append({
                    'name': 'zero_agent/reflect/' + f,
                    'cmd': [sys.executable, 'zero_agent', '--reflect', 'zero_agent/reflect/' + f],
                })
    ...
    if os.path.isdir(frontends_dir):
        for f in sorted(os.listdir(frontends_dir)):
            if 'app' in f and f.endswith('.py') and f not in EXCLUDES:
                if 'stapp' in f: cmd = [sys.executable, '-m', 'streamlit', 'run', 'zero_agent/bots/' + f, '--server.headless=true']
                else: cmd = [sys.executable, 'zero_agent/bots/' + f]
                services.append({'name': 'zero_agent/bots/' + f, 'cmd': cmd})
    return services
