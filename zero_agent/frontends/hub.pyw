# hub.pyw - ZeroAgent Service Hub (launcher GUI)
...
LOCK_PORT = 19735
# hub.pyw is at zero_agent/frontends/hub.pyw under the ZeroAgent project root
...
def discover_services():
    """Discover ZeroAgent services: reflect tasks and bot frontends."""
    services = []

    # Reflect tasks — modules in zero_agent/reflect/ (*.py, not starting with _)
    reflect_dir = os.path.join(BASE_DIR, 'zero_agent', 'reflect')
            ...
            if f.endswith('.py') and not f.startswith('_'):
                services.append({
                    ...
                    'cmd': [sys.executable, '-m', 'zero_agent.runners.cli',
                            '--reflect', 'zero_agent/reflect/' + f],
                })

    # Bot frontends — modules in zero_agent/bots/ (*_app.py)
    bots_dir = os.path.join(BASE_DIR, 'zero_agent', 'bots')
    if os.path.isdir(bots_dir):
        for f in sorted(os.listdir(bots_dir)):
            if f.endswith('_app.py') and not f.startswith('_'):
                modname = f[:-3]  # drop .py
                services.append({
                    'name': 'bots/' + f,
                    'cmd': [sys.executable, '-m', 'zero_agent.bots.' + modname],
                })
...
class LauncherApp:
    def __init__(self, root):
        self.root = root
        self.root.title('ZeroAgent Launcher')
...
if __name__ == '__main__':
    ...
    if lock is None:
        try:
            ...
            r = tk.Tk()
            r.withdraw()
            mb.showinfo('ZeroAgent Launcher', 'Already running.')
            r.destroy()
        except Exception:
            pass
        sys.exit(0)
