...
# Must run before importing the agent runtime. We capture the raw binary stdout
# for ACP JSON-RPC, then redirect the text-mode stdout to stderr so any stray
# prints from runtime modules don't pollute the ACP channel.
...
class _StdoutToStderrRouter(io.TextIOBase):
    """Redirect text-mode stdout to stderr so runtime prints don't leak."""
...
try:
    from .za_adapter import AgentRunner, create_agent
except ImportError:
    from za_adapter import AgentRunner, create_agent
...
class ZeroAgentAcpBridge:
    ...
    def new_agent(self) -> AgentRunner:
        agent = create_agent()
...
GenericAgentAcpBridge = ZeroAgentAcpBridge
