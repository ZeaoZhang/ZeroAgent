"""世界线(Rewind / checkpoint)统一后端 —— **UI 无关,单文件,tui/gui 共用**。
...
# ============================================================================
# rich.cells fallback (no hard dependency on rich)
# ============================================================================
try:
    from rich.cells import cell_len as _rc_cell_len, set_cell_size as _rc_set_cell_size
except ImportError:
    import unicodedata

    def _rc_cell_len(s: str) -> int:
        """Fallback display width: CJK characters count as 2."""
        width = 0
        for ch in s:
    ...
    def _rc_set_cell_size(s: str, total: int) -> str:
        """Fallback: truncate/pad to given display width."""
        if not s:
            return " " * total
        result: list[str] = []
        used = 0
...
def ellipsize(s: str, max_cells: int = _TITLE_MAX_CELLS) -> str:
    ...
    s = (s or "").replace("\n", " ").strip()
    if _rc_cell_len(s) <= max_cells:
        return s
    return _rc_set_cell_size(s, max(1, max_cells - 1)).rstrip() + "…"
