"""ZeroAgent ACP Bridge — Agent Communication Protocol v1 over stdio JSON-RPC.

适配 ZeroAgent/AgentRunner, 替代 GenericAgent 的 ACP bridge。

Usage:
    python -m zero_agent.frontends.acp_bridge [--llm-no N]
"""
...
# —— stdout 重定向 (必须在任何 agent import 之前) ——

# 捕获原始 stdout fd 用于 ACP JSON-RPC 输出, text-mode stdout 重定向到 stderr
...
class ZeroAgentAcpBridge:
    ...
    def new_agent(self) -> tuple[ZeroAgent, AgentRunner]:
        za = ZeroAgent()
        runner = AgentRunner(za)
        if self.llm_no > 0:
            runner.next_llm(self.llm_no)
    ...
    def handle_session_new(self, req_id: Any, params: dict[str, Any]) -> None:
        ...
        session_id = f"za_{uuid.uuid4().hex}"
        za, runner = self.new_agent()
        session = SessionState(session_id=session_id, cwd=cwd, agent=za, runner=runner)
        self._sessions[session_id] = session
        self.write_message(
            jsonrpc_result(
                req_id,
                {"sessionId": session_id, "modes": None, "configOptions": None},
            )
        )
    ...
    def handle_session_prompt(self, req_id: Any, params: dict[str, Any]) -> None:
        ...
        if not prompt_text:
            self.write_message(
                jsonrpc_error(-32602, "prompt must contain text or supported content", req_id)
            )
            return
        ...
        def run_prompt() -> None:
            ...
            try:
                dq = session.runner.put_task(prompt_text, source="acp")
    ...
    def _drain_agent_queue(self, session: SessionState, dq: queue.Queue) -> None:
        sent_any = False
        while True:
            item = dq.get()
    ...
    def handle_message(self, msg: dict[str, Any]) -> None:
        ...
        try:
            if method == "initialize":
                ...
                if req_id is not None:
                    self.write_message(
                        jsonrpc_error(-32601, f"method not found: {method}", req_id)
                    )
        except Exception as exc:
            eprint("[ZeroAgent ACP] request handler failed:", traceback.format_exc())
            if req_id is not None:
                self.write_message(
                    jsonrpc_error(
                        -32603, f"internal error: {type(exc).__name__}: {exc}", req_id,
                    )
                )

    def serve(self) -> None:
        eprint("[ZeroAgent ACP] bridge started")
        stdin = (
        ...
        for raw_line in stdin:
            ...
            if self._shutdown:
                break
        eprint("[ZeroAgent ACP] bridge stopped")


def main() -> int:
    parser = argparse.ArgumentParser(description="ZeroAgent ACP bridge over stdio")
    parser.add_argument("--llm-no", type=int, default=0, help="LLM index for ZeroAgent")
    args = parser.parse_args()
    bridge = ZeroAgentAcpBridge(llm_no=args.llm_no)
    bridge.serve()
    return 0
