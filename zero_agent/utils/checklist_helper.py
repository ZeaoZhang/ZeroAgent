...
class CL:
    ...
    def start_worker(self, n=None):
        ...
        if n <= 0: return
        for i in range(n):
            subprocess.Popen(_CLI + [
                "--reflect", str(_W_RE),
                "--reflect-arg", f"base_url={self.bbs_url}",
                "--reflect-arg", f"board_key={self.bbs_key}",
                "--reflect-arg", f"name=w{i+1}",
            ], **_PK)
    ...
    def start_master(self):
        ...
        if old_pid and self._pid_alive(old_pid):
            ...
            return
        p = subprocess.Popen(_CLI + [
            "--reflect", str(_M_RE),
            "--reflect-arg", f"mr_folder={self.folder.resolve()}",
        ], **_PK)
