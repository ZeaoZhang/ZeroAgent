...
def _env(name, default=None):
    """Read env var with ZA_ prefix, falling back to GA_ prefix for backward compatibility."""
    val = os.environ.get("ZA_" + name)
    if val is not None:
        return val
    val = os.environ.get("GA_" + name)
    if val is not None:
        return val
    return default

# File is at zero_agent/assets/ga_ultraplan.py; three dirname calls reach the ZeroAgent project root.
_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
_PORT = int(_env("ULTRAPLAN_PORT", "47831"))
...
_RUN_DIR = os.path.abspath(_env("ULTRAPLAN_RUNDIR", os.path.join(_ROOT, "temp", "ultraplan_default")))
...
def _task_slug(path):
    ...
    if not re.search(r"[a-zA-Z]", stem):
        stem = os.path.basename(os.path.dirname(os.path.abspath(path or "."))) or stem
...
class _H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        with _exec_lock, redirect_stdout(out), redirect_stderr(err):
            ...
            cwd = os.getcwd(); old_env = os.environ.copy(); os.environ["ZA_ULTRAPLAN_DAEMON"] = "1"; _PLANNED = False; _TASK_SLUG = req.get("task") or _task_slug(req.get("path")); _save_session()
            try:
...
def _serve_daemon():
    global _srv
    sys.modules.setdefault("zero_agent.assets.ga_ultraplan", sys.modules[__name__])
    _srv = ThreadingHTTPServer(("127.0.0.1", _PORT), _H); _srv.timeout = 60; url = f"http://127.0.0.1:{_PORT}/"
    print(f"[ultraplan] {url}", flush=True)
    if _env("ULTRAPLAN_BROWSER", "1") != "0": webbrowser.open(url)
...
def _show():
    if _env("ULTRAPLAN_DAEMON") == "1" or _env("ULTRAPLAN_HTML") == "0": return
    if not _ping():
        subprocess.Popen([sys.executable, __file__, "--daemon"], cwd=_ROOT, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env={**os.environ, "ZA_ULTRAPLAN_DAEMON":"1"})
        for _ in range(20):
            if _ping(): break
            sleep(0.25)
...
def plan(rundir):
    ...
    if _env("ULTRAPLAN_DAEMON") == "1": return
