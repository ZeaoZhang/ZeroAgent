...
DEFAULT_BACKEND = os.environ.get('ZA_VISION_BACKEND', 'claude')
# =================================================================
    ...
    try:
        if backend == 'claude':
            ...
            return _call_openai_compat(
                ...
                apikey=os.environ.get('MODELSCOPE_API_KEY', ''),
...
def _call_claude(b64, prompt, timeout, max_tokens=1024):
    ...
    resp = requests.post(
        os.environ.get('ANTHROPIC_API_BASE', 'https://api.anthropic.com').rstrip('/') + '/v1/messages',
        json={'model': os.environ.get('ZA_VISION_CLAUDE_MODEL', 'claude-sonnet-4-6'), 'max_tokens': max_tokens, 'messages': [{
            'role': 'user',
            'content': [
