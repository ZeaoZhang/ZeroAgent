...
```
任意入口 (CLI / bot / desktop)
   └─ bots/shared/review_cmd.py   ← 命令分发,剥 "/review" 前缀,注入 user_request
       └─ zero_agent/assets/review_sop/review_inline_prompt.txt   ← 完整 in-session 协议
           └─ memory/sops/code_review_principles.md         ← 15 条好代码原则
```

- `review_cmd.py:handle()` —— 解析 `/review` 参数并注入 `user_request`
- `review_cmd.py:_render_prompt()` —— 加载 prompt 模板,注入 `{user_request}` + `{project_root}`
...
`file_read("memory/sops/code_review_principles.md")` —— 15 条好代码原则,**每条 finding 必须
能映射到其中一条**。
...
> 每条规则的展开详见 `zero_agent/assets/review_sop/review_inline_prompt.txt` §5。
...
> 展开详见 `zero_agent/assets/review_sop/review_inline_prompt.txt` §6。
...
- **自定义评审条目**:编辑 `memory/sops/code_review_principles.md`,reviewer 启动时整段注入
