...
**Python 调用方式:**
```python
from zero_agent.utils.procmem_scanner import scan_memory
...
```powershell
# 基础搜索
python -m zero_agent.utils.procmem_scanner <PID> "pattern" --mode string

# LLM 增强模式（输出包含上下文的 JSON，推荐）
python -m zero_agent.utils.procmem_scanner <PID> "pattern" --llm
```
