"""Feishu (飞书) Bot 前端 for ZeroAgent.
...
Usage:
    python -m zero_agent.bots.feishu_app
"""
...
def _claim_message_once(message_id):
    ...
    with _DEDUP_LOCK:
        ...
        if len(_SEEN_MESSAGES) > _DEDUP_MAX:
            oldest = sorted(_SEEN_MESSAGES.items(), key=lambda item: item[1])[:len(_SEEN_MESSAGES) - _DEDUP_MAX]
            for mid, _ in oldest:
                _SEEN_MESSAGES.pop(mid, None)
...
# —— Post / Interactive content extraction ——
def _extract_interactive_content(content):
    ...
    for row in content.get("elements", []) or []:
        ...
        for field in element.get("fields", []) or []:
            if isinstance(field, dict):
                ...
                if isinstance(field_text, dict):
                    c = field_text.get("content", "") or field_text.get("text", "")
                    if c:
                        parts.append(c)
    elif tag == "a":
...
def _download_file_sync(message_id, file_key, resource_type="file"):
    ...
    try:
        request = (
            GetMessageResourceRequest.builder()
            .message_id(message_id).file_key(file_key).type(resource_type).build()
        )
        response = client.im.v1.message_resource.get(request)
        if response.success():
            data = response.file.read() if hasattr(response.file, "read") else response.file
    ...
    if ext in _IMAGE_EXTS:
        ...
        if image_key:
            send_message(
                ...
                msg_type="image", receive_id_type=receive_id_type,
            )
            return True
    else:
        ...
        if file_key:
            ...
            send_message(
                ...
                msg_type=msg_type, receive_id_type=receive_id_type,
            )
            return True
...
def _build_user_message(message):
    ...
    if msg_type == "text":
        ...
        if msg_type == "share_chat":
            sub_parts.append(f"[shared chat: {content_json.get('chat_id', '')}]")
        elif msg_type == "share_user":
            sub_parts.append(f"[shared user: {content_json.get('user_id', '')}]")
        elif msg_type == "interactive":
            sub_parts.extend(_extract_interactive_content(content_json))
        elif msg_type == "share_calendar_event":
            sub_parts.append(f"[shared calendar event: {content_json.get('event_key', '')}]")
        elif msg_type == "system":
            sub_parts.append("[system message]")
        elif msg_type == "merge_forward":
            sub_parts.append("[merged forward messages]")
        parts.append("\n".join([p for p in sub_parts if p]).strip() or f"[{msg_type}]")
    else:
...
class FeishuApp(AgentBotMixin):
    ...
    async def run_agent(self, chat_id, text, *, receive_id=None, receive_id_type="open_id", images=None, **_):
        ...
        try:
            ...
            if getattr(za, "_fs_active_task_id", None) == task_id:
                try:
                    delattr(za, "_fs_active_task_id")
                except AttributeError:
                    pass
            za._turn_end_hooks.pop(hook_key, None)
...
def get_app():
    ...
    if app_instance is None:
        app_instance = FeishuApp(runner, user_tasks)
    return app_instance
...
def check_config(init_agent=False):
    ...
    if init_agent:
        ...
        result["llm_count"] = len(runner.list_llms())
        result["current_llm"] = runner.get_llm_name() if runner.llmclient else ""
    return result
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ZeroAgent Feishu frontend")
