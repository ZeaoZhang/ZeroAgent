"""Telegram Bot 前端 for ZeroAgent.

使用 python-telegram-bot 库。适配 ZeroAgent 的 AgentRunner 接口。

Usage:
    python -m zero_agent.bots.telegram_app
"""
...
ALLOWED = set(_KEYS.get("tg_allowed_users", []))
    ...
    if event.get("multi") and action.startswith(f"{_ASK_TOGGLE_ACTION}:"):
        ...
        return await query.edit_message_reply_markup(
            reply_markup=_build_ask_user_markup(
                menu_id, candidates, multi=True, selected_indexes=stored["selected"],
            )
        )
...
async def handle_llm_callback(update, ctx):
    ...
    try:
        runner.next_llm(selected_idx)
        selected_name = runner.get_llm_name()
    except Exception as exc:
...
async def cmd_llm(update, ctx):
    ...
    if len(args) > 1:
        try:
            n = int(args[1])
            runner.next_llm(n)
            await update.message.reply_text(f"✅ 已切换到 [{runner.llm_no}] {runner.get_llm_name()}")
        except (ValueError, IndexError):
            await update.message.reply_text(f"用法: /llm <0-{len(runner.list_llms()) - 1}>")
    else:
...
async def handle_command(update, ctx):
    ...
    if op == "/new":
        _cancel_stream_task(ctx)
        return await update.message.reply_text(reset_conversation(runner))
    if op == "/restore":
        _cancel_stream_task(ctx)
        try:
    ...
    if op == "/continue":
        if cmd != "/continue":
            _cancel_stream_task(ctx)
        return await update.message.reply_text(handle_frontend_command(runner, cmd))
    return await update.message.reply_text(HELP_TEXT)
